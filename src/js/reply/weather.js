...
const weatherMsg = {
  ...
  async message (city) {
    ...
    if (isTimeout(resData)) {
      ...
      text = `${resData.city}：${todayWea.wea}，当前温度${todayWea.tem}，白天温度${todayWea.tem1}，晚上温度${todayWea.tem2}，湿度${todayWea.humidity}%，风速：${todayWea.win_speed}，${idx === 1 ? '运动指数' : todayWea.index[idx].title}：${todayWea.index[idx].desc}更新时间：${moment(resData.update_time).format('MM月DD日HH时mm分')}${thanksWords[randomWordsIdx]}`
    }
    return text
  }
}
